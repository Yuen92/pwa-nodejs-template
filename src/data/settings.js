// Minimum datas for iPhone 4
// 320x480
// 6 Settings to display
export const properties = {
    "description" : "Settings page which list all the settings features for your amazing progressive web application Data-Driven Technological Innovations.",
    "settings": [
        {
            "iconPath": "M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z",
            "iconName": "home",
            "href": "/settings/app-install",
            "name": "A2HS - Application Install"
        },
        {
            "iconPath": "M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM17 13l-5 5-5-5h3V9h4v4h3z",
            "iconName": "cloudDownload",
            "href": "/settings/load-screens",
            "name": "Load All Screens"
        },
        {
            "iconPath": "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z",
            "iconName": "locationOn",
            "name": "Enable Location"
        },
        {
            "iconPath": "M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z",
            "iconName": "fullscreen",
            "name": "Enable Fullscreen"
        },
        {
            "iconPath": "M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z",
            "iconName": "call",
            "name": "Call"
        },
        {
            "iconPath": "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z",
            "iconName": "accountCircle",
            "name": "Auto Sign-in"
        }
    ]
}